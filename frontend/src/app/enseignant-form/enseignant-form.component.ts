...
export class EnseignantFormComponent {
  ...
  initForm2(enseignant: Member): void {
    ...
    this.form = new FormGroup({
      ...
      dateNaissance: new FormControl(enseignant.dateNaissance, [
        Validators.required,
      ]),
  ...
  OnSubmit(): void {
    ...
    if (!!this.idCourant1) {
      ...
      this.MS.updateMember('enseignant', enseignant).subscribe(() => {
        this.router.navigate(['/members']);
      });
    } else {
      this.MS.saveMember('enseignant', enseignant).subscribe(() => {
        this.router.navigate(['/members']);
      });
    }
  }
