...
export class EtudiantFormComponent implements OnInit {
  ...
  initForm2(etudiant: Member): void {
    this.form = new FormGroup({
      ...
      dateNaissance: new FormControl(etudiant.dateNaissance, [
        Validators.required,
      ]),
      ...
      dateInscription: new FormControl(etudiant.dateInscription, [
        Validators.required,
      ]),
  ...
  OnSubmit(): void {
    ...
    if (!!this.idCourant1) {
      ...
      this.MS.updateMember('etudiant', etudiant).subscribe(() => {
        this.router.navigate(['/members']);
      });
    } else {
      this.MS.saveMember('etudiant', etudiant).subscribe(() => {
        this.router.navigate(['/members']);
      });
    }
  }
